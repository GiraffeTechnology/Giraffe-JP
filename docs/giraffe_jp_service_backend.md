...
Giraffe JP is a made-to-order platform layer built on top of the existing abcdYi B2M coordination system. It introduces four backend modules:

| Module | Iteration | Purpose |
|---|---|---|
| Service Core | 01 | Service nodes, confirmation requests, customer-service tasks, P0 blocking, execution graph |
...
alembic/versions/
  b2c3d4e5f6a7_add_giraffe_jp_service_core.py
  d4e5f6a7b8c9_add_giraffe_jp_iterations_02_03_04.py
```

### Database tables

All top-level Giraffe JP tables include `tenant_id` as a foreign key to `tenants.id`. Service-core tables are created by migration `b2c3d4e5f6a7` (revises `a1b2c3d4e5f6`). Iteration 02/03/04 tables are created by migration `d4e5f6a7b8c9` (revises `b2c3d4e5f6a7`).

| Table | Description | Migration |
|---|---|---|
| `giraffe_jp_service_nodes` | Qualified production partner network nodes | `b2c3d4e5f6a7` |
| `giraffe_jp_confirmation_requests` | Service confirmation requests | `b2c3d4e5f6a7` |
| `giraffe_jp_customer_service_tasks` | Proactive customer service tasks | `b2c3d4e5f6a7` |
| `giraffe_jp_message_category_permissions` | Per-tenant auto-send permissions; unique on `(tenant_id, category_id)` | `d4e5f6a7b8c9` |
| `giraffe_jp_conversation_threads` | Communication threads (one per party/project/thread-type) | `d4e5f6a7b8c9` |
| `giraffe_jp_messages` | Individual messages (INBOUND or OUTBOUND) within a thread | `d4e5f6a7b8c9` |
| `giraffe_jp_outbound_message_drafts` | Outbound drafts with approval status tracking | `d4e5f6a7b8c9` |
| `giraffe_jp_message_delivery_logs` | Delivery log records (simulated in this iteration) | `d4e5f6a7b8c9` |
| `giraffe_jp_formalwear_order_profiles` | Per-project formalwear order profiles | `d4e5f6a7b8c9` |
| `giraffe_jp_c2b2m_role_edges` | C2B2M role relationship edges for a project | `d4e5f6a7b8c9` |
